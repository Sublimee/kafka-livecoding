...
Kafka предоставляет 2 механизма удаления данных (_cleanup.policy_):
* retention по времени/размеру (_delete_, включен по умолчанию)
* log compaction использует ключи для определение необходимости удаления определенных данных (_compact_)

Эти механизмы могут быть активированы одновременно: _cleanup.policy=delete, compact_.
